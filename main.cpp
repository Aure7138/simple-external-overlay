...
bool printModuleBaseAddress(HANDLE handle, DWORD pid, std::map<std::wstring, DWORD64>& moduleBaseAddressList) {
	HMODULE hMods[4096];
	DWORD cbNeeded;
	...
	do
	{
		...
		moduleBaseAddressList.insert(std::pair<std::wstring, DWORD64>(me32.szModule, (DWORD64)(me32.modBaseAddr)));
...
int main()//int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PWSTR pCmdLine, int nCmdShow)
{
	...
	// ´æ´¢»ùÖ·Æ«ÒÆ
	std::map<std::wstring, std::wstring> baseStringList = {
		//  ÊÇ·ñÃé×¼
		{L"isCollimation",L"GTA5.exe+20DE250"},
		{L"collimationPed",L"GTA5.exe+20DE240"}
	};
	// ´æ´¢»ùÖ·
	std::map<std::wstring, DWORD64> baseAddressList = {};
	// ´æ´¢Ä£¿é»ùÖ·
	std::map<std::wstring, DWORD64> moduleBaseAddressList = {};
	printModuleBaseAddress(handle, process_id, moduleBaseAddressList);
	for (std::map<std::wstring, std::wstring>::iterator it = baseStringList.begin(); it != baseStringList.end(); it++)
	{
		std::wstring code = it->second;
		int index = code.find(L"+");
		std::wstring name = code.substr(0, index);
		baseAddressList[it->first] = wcstol((code.substr(index + 1).c_str()), 0, 16) + moduleBaseAddressList[name];
	}
	isCollimationAddress = baseAddressList[L"isCollimation"];
	collimationPedAddress = baseAddressList[L"collimationPed"];
